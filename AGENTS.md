...
Commit과 Push는 반드시 다음 2단계 승인을 거친다.

1. 사용자가 `커밋해`라고 말하면 즉시 Commit하거나 Push하지 않는다.
...
4. 이 확인 보고 직후 사용자가 `해`라고 말한 경우에만 Commit 후 Push한다.

다른 질문이나 작업에 대한 `해`는 Commit 승인으로 해석하지 않는다. Codex가 커밋 시점을 제안했더라도 `커밋해`와 `해` 승인을 생략하지 않는다.
...
- Commit이 실패하면 Push하지 않고 오류를 보고한다.
- Push가 실패하면 로컬 Commit을 유지하고 오류를 보고한다.
- Push 거부 시 자동으로 Pull, Merge 또는 Rebase하지 않는다.
