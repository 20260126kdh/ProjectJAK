...
# 2026-08-24 - 사망 전환 최종 배경 이미지

- 전달받은 혈흔 이미지를 `Death_Background.png` 공통 사망 배경으로 추가했다.
- 붉은 확산 완료 후 기존 사망 UI와 함께 배경이 페이드 인하도록 연결했다.
- Captain, Physique, Technician 프리팹에 동일한 배경을 연결했다.
- 기존 사망 문구, 진행 스테이지, 클래스별 사망 이미지와 타이틀 버튼 배치는 유지했다.
# 2026-08-24 - 플레이어 사망 UI 단일 패널 개편

- 관련 경로: `Assets/Scripts/UI/PlayerDeathTransitionController.cs`
- 기존 사망 문구·최종 진행도 가로 배너 2개를 신규 2:3 세로 패널로 통합했다.
- `imagegen`으로 해양 고딕풍의 투명 배경 패널 `Death_SummaryPanel.png`을 제작하고 세 플레이어 Prefab에 연결했다.
- 프레임 뒤에 완전 불투명한 검붉은 내부판을 추가해 혈흔 배경이 패널 안으로 비치지 않도록 보강했다.
- 패널을 3:4에 가까운 비율로 확대하고 사망 문구·진행도·타이틀 버튼의 크기와 명암 대비를 높였다.
- 전투 종류를 일반 흰색, 보스 붉은색, 진 보스 금색으로 구분했다.
- `Death_SummaryPanel.png`의 핏자국과 붉은 얼룩을 제거하고 마른 녹·산화 철·긁힘 질감으로 교체했다.
- 프레임 외곽의 투명 알파와 어두운 중앙 글자 영역은 유지했다.
- 진행도에 현재 스테이지, 해당 스테이지의 전투 순서, 일반·보스·진 보스 전투 구분을 추가했다.
- 3스테이지 아리엘 전투는 모르바엘 다음 전투 번호와 `진 보스 전투`로 표시한다.
- 정적 검증 완료, Unity Play Mode 시각 확인 필요.
# 2026-08-24 - 독립 시뮬레이터 사망 후 중단 수정

- 관련 경로: `PlayerCombat.cs`, `TitleManager.cs`, `PolishCampaignController.cs`
- 자동 캠페인 중 일반 사망 연출을 시작하지 않아 TimeScale 정지와 타이틀 복귀 경로가 다음 Run 전환과 경쟁하지 않도록 수정했다.
- 기존 자동 세션이 살아 있는 상태의 타이틀 중복 시작을 차단했다.
- 실행 중 타이틀 씬 진입을 감지하면 현재 Run의 클래스 선택 씬으로 복구하도록 보강했다.
- `Session_20260824_100216` 분석 기준 PHY-018 저장 후 PHY-019 시작 단계의 중복 시뮬레이션 실패를 대상으로 한다.

# 2026-08-24 - 클래스 선택 배너·배경 개편

- 관련 경로: `Assets/Resources/UI/ClassSelect`, `ClassSelectManager.cs`, `ClassSelectionCardUI.cs`
- 기존 캐릭터 휴식 일러스트를 참고해 피지크·테크니션·캡틴 전용 세로 배너를 각각 제작했다.
- 클래스별 붉은 근접전, 녹색 정밀 장비, 보라색 원혼 지휘 특징을 배너에 반영했다.
- 클래스 선택 목록 뒤에 폭풍우 선박 갑판 공용 배경을 추가했다.
- 배너와 배경은 `AspectRatioFitter.EnvelopeParent`와 UI Mask로 비율을 유지하며 영역을 채우도록 구성했다.
- 기존 선택 확대, 상세 영상, 영상 블러 및 확인 흐름은 변경하지 않았다.
- 정적 검증 완료, Unity Play Mode 시각 확인 필요.

# 2026-08-24 - 클래스 선택 배너 스타일·전환 보강

- 피지크 배너의 얼굴 비율, 선화와 피부 명암을 기준으로 테크니션·캡틴 얼굴을 다시 제작했다.
- 각 카드의 이미지 영역을 명판과 분리하고 `PHYSIQUE`, `TECHNICIAN`, `CAPTAIN` 영문 이름을 추가했다.
- 이미지 외곽에 4px 클래스색·황동 혼합 테두리와 어두운 금속 명판을 적용했다.
- 클래스 클릭 시 선택 이미지를 전체 화면으로 확대하거나 다른 카드를 숨기던 동작을 제거했다.
- 상세 패널은 기존 영상과 블러 구성을 유지하며 Fade In과 약한 Scale In으로 나타나도록 변경했다.
- `Assembly-CSharp` 빌드 결과 오류 0개, 기존 프로젝트 경고 4개를 확인했다.
- Unity Play Mode 시각 확인 필요.

# 2026-08-24 - 클래스 선택 배너 호버·표시 정리

- 기존 버튼 중앙의 클래스 텍스트를 런타임에서 식별해 숨기고 하단 영문 명판만 유지했다.
- 클래스 클릭 후 상세 패널 전환 시작 전 0.4초 대기 시간을 추가했다.
- 마우스 호버 시 카드 전체가 3.5% 확대되고 0.15초 동안 부드럽게 복귀하도록 적용했다.
